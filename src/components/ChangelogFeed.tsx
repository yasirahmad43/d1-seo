...
// Pill styles for the dark D1 surface. No violet/purple — only blues, sky,
// emerald, amber, and zinc.
...
export default function ChangelogFeed({ entries }: { entries: Entry[] }) {
  if (!entries.length) return <p className="text-sm text-slate-500">No changelog entries yet.</p>;
  return (
        ...
        return (
          <li key={e.id} className="card">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-slate-500">{relativeDate(e.occurred_at)}</span>
            </div>
            <div className="font-medium">{e.title}</div>
            {e.description && <p className="text-sm text-slate-600 mt-1">{e.description}</p>}
            {e.related_url && (
              <a href={e.related_url} target="_blank" rel="noreferrer" className="text-xs text-brand hover:underli